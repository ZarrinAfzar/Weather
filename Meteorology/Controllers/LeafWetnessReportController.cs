...
{
    [Authorize(Roles = "AdminiStratore,LeafWetnessReport")]
    public class LeafWetnessReportController : Controller
    {

        private readonly GenericUoW _genericUoW;
        private readonly UserManager<User> _userManager;
        public LeafWetnessReportController(GenericUoW genericUoW, UserManager<User> userManager)
        {
        ...
        public IActionResult LeafWetnessReport_List(int stationId, string startDate, string endDate, double Max)
        {
            //ViewBag.Max = max;
            //ViewBag.Min = min;
            DataTable dt;
            try
            {
                ...
                Parametr[3, 0] = "@Leaf";
                Parametr[3, 1] = Max.ToString();

                dt = _genericUoW.GetFromSp(Parametr, "Sp_LeafWetnessReport");
                double SumData = 0;
                
                dt.Columns.Add("مجموع ساعات خیسی برگ", typeof(double));

                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    SumData += Convert.ToDouble(string.Format("{0:0.####}", dt.Rows[i]["Data"]));
                    dt.Rows[i]["مجموع ساعات خیسی برگ"] = SumData;
        ...
        [HttpGet]
        public IActionResult ExportToExcel(int stationId, string stationname, string startDate, string endDate, double Max)
        {
            ...
            DataTable dt = _genericUoW.GetFromSp(Parametr, "Sp_LeafWetnessReport");
            double SumData = 0;
            if (dt.Rows.Count > 0)
            {
                dt.Columns.Add("تاریخ", typeof(string));
                dt.Columns.Add("تعداد ساعات روزانه خیسی برگ", typeof(double));
                dt.Columns.Add("مجموع ساعات خیسی برگ", typeof(double));

                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    dt.Rows[i]["تاریخ"] = Convert.ToDateTime(dt.Rows[i]["DateTime"]).ToPeString("yyyy/MM/dd");
                    dt.Rows[i]["تعداد ساعات روزانه خیسی برگ"] = Convert.ToDouble(dt.Rows[i]["Data"]);
                    SumData += Convert.ToDouble(string.Format("{0:0.####}", dt.Rows[i]["Data"]));
                    dt.Rows[i]["مجموع ساعات خیسی برگ"] = SumData;
